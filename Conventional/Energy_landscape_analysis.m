...
%% Within Participants %%
    for i=1:length(participants)
        for j=1:length(sessions)
            ...
            Param.InputFile=['SampleData_Binarized_Subject_' participants(i) '_Session_' sessions(j) '.mat'];
            Param.DataType=2;
...
%% Between Participants %%
    for i=1:length(sessions)
        for j=1:length(participants)
            ...
            Param.Y=i;
            Param.ProcessingType=1;
            Param.InputFile=['SampleData_Binarized_Subject_' participants(j) '_Session_' sessions(i) '.mat'];
            Param.DataType=2;
            Param.fReadBasinData=false;
            Param.BasinDataFile='BasinData_test.mat';
            Param.OutputFolder=out_folder;
